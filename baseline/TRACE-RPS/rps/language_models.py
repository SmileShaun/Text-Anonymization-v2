...
class GPT:
    ...
    def __init__(self, model_name):
        ...
        if 'gpt' in model_name:
            self.client = OpenAI(api_key="",base_url="",default_headers = {"x-foo": "true"}, http_client=httpx.Client())
            ...
            for _ in range(self.API_MAX_RETRY):
                try:
                    ...
                    if response.choices[0].logprobs.content is None:
                        raise ValueError("logprobs.content is None")
                    response_logprobs = [
                    ...
                    break
                except (openai.OpenAIError, ValueError, TypeError, AttributeError) as e:
                    print(type(e), e)
...
class HuggingFace:
    def __init__(self,model_name, model, tokenizer):
        ...
        if 'llama2' in self.model_name.lower() or 'vicuna' in self.model_name.lower():
            self.pos_to_token_dict = {v: k.replace('▁', ' ') for k, v in self.tokenizer.get_vocab().items()}
        elif 'qwen' in self.model_name.lower() or 'llama3' in self.model_name.lower() or 'deepseek' in self.model_name.lower():
            self.pos_to_token_dict = {v: k.replace('Ġ', ' ') for k, v in self.tokenizer.get_vocab().items()}
        else:
