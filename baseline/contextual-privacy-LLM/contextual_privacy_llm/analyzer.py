...
class PrivacyAnalyzer:
    ...
    def __init__(self, 
        ...
        if self.llm_backend == 'ollama':
            ...
            self.api_base_url = (api_base_url or os.getenv('OPENAI_BASE_URL', '')).rstrip('/')
            self.api_key = api_key or os.getenv('OPENAI_API_KEY', '')
            if not self.api_base_url:
                raise ValueError("api_base_url is required when llm_backend='api'")
            if not self.api_key:
                raise ValueError("api_key is required when llm_backend='api'")
            self.host = None
            self.api_url = None
        else:
            raise ValueError(f"Unknown llm_backend: {llm_backend!r}. Use 'ollama' or 'api'.")
    ...
    def _call_llm(self, prompt: str) -> Union[str, None]:
        """Dispatch to Ollama or OpenAI-compatible chat API."""
    ...
    def _call_openai_api(self, prompt: str) -> Union[str, None]:
        ...
        try:
            ...
            logger.debug(f"Error calling OpenAI-compatible API: {e}")
            return None
    ...
    def detect_sensitive_info(self, text: str, intent: str, task: str, conversation_id: str) -> Tuple[str, Dict]:
        ...
        def get_context(prompt_key: str) -> Tuple[str, List[str]]:
            ...
            response = self._call_llm(self.prompts[prompt_key].format(
