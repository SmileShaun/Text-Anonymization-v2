#!/usr/bin/env python3
"""
Causal frozen-history anonymization driver for llm-anonymization (ICLR 2025 / eth-sri).
...
python /home/zxz/project/Text-Anonymization/baseline/llm-anonymization/zxz_causal_frozen_anonymize.py \
  --backend api \
  --baseline-repo /home/zxz/project/Text-Anonymization/baseline/llm-anonymization \
  ...
  --output-dir /home/zxz/project/Text-Anonymization/baseline/llm-anonymization/results/a_deepseek-chat_i_deepseek-chat \
...
CUDA_VISIBLE_DEVICES=2,3 python /home/zxz/project/Text-Anonymization/baseline/llm-anonymization/zxz_causal_frozen_anonymize.py \
  --backend vllm \
  --baseline-repo /home/zxz/project/Text-Anonymization/baseline/llm-anonymization \
  --profiles-dir /home/zxz/project/Text-Anonymization/data/synthpai/profiles \
  --profile-list /home/zxz/project/Text-Anonymization/data/synthpai/top30_most_comments.txt \
  --output-dir /home/zxz/project/Text-Anonymization/baseline/llm-anonymization/results/a_Qwen3-14B_i_Qwen3-14B \
...
python /home/zxz/Text-Anonymization/baseline/llm-anonymization/zxz_causal_frozen_anonymize.py \
  --backend api \
...
_BASELINE_PKG_DIR = DEFAULT_BASELINE_REPO.parent
...
BASELINE_NAME = "llm-anonymization"
PII_KEY_MAP = {
    "income_level": "income",
    "income": "income",
    "age": "age",
    ...
    *,
    ...
    with track_token_usage(raw_profile.author) as token_usage:
        ...
        write_result(
            ...
            model_name=model.config.name if model is not None else None,
            max_refinement_rounds=max_refinement_rounds,
            anonymizer_type=anonymizer_type,
        )
    ...
    for attempt in range(1, retries + 2):
        try:
            ...
            err_entry: Dict[str, Any] = {
                ...
                "round": round_idx,
                "error": message,
                "error_kind": classify_error_kind(message, exc_type=type(exc)),
            }
            if isinstance(exc, CommentAlignmentError):
...
def write_result(
    *,
    ...
    soft_mismatch_comments: List[Dict[str, Any]] = []
    for row in rows:
        status = str(row.get("status", "success"))
        status_counts[status] = status_counts.get(status, 0) + 1
        if row.get("count_mismatch") and not row.get("max_retries_exhausted"):
            soft_mismatch_comments.append(
                {
                    "index": row.get("index"),
                    "status": status,
    ...
    summary: Dict[str, Any] = {
        ...
        "soft_count_mismatch_comments": len(soft_mismatch_comments),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Causal frozen-history driver for llm-anonymization on SynthPAI "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"Default: {DEFAULT_PROFILE_LIST}."
        ),
    )
