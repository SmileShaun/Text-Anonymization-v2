#!/usr/bin/env python3
"""
SynthPAI 匿名化调度（DeepSeek deepseek-chat）——保留原版 25 条 comments 截断。

完全复用 eth-sri/llm-anonymization 的 ``run_anonymized``，不修改算法逻辑。
配置对齐 ``configs/anonymization/synthpai/synthpai_gpt_4.yaml``，仅将模型替换为
...
默认输入（由 zxz_build_synthpai_input_datasets.py 生成）：
  data/inputs/synthpai_truncated_comments/profiles.jsonl

默认输出：
  results/synthpai_llm-anonymization_250pers/deepseek-chat-truncated/

用法示例

python zxz_synthpai_deepseek_anonymize_truncated.py \
...
DEFAULT_OUTPATH = (
    REPO_ROOT / "results" / "synthpai_llm-anonymization_250pers" / "deepseek-chat-truncated"
)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run original llm-anonymization SynthPAI pipeline with DeepSeek "
            "deepseek-chat, keeping the 25-comment truncation."
        )
    )
    parser.add_argument(
        "--api-key",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: data/inputs/synthpai_truncated_comments/profiles.jsonl."
        ),
    )
    parser.add_argument(
        "--outpath",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print("=" * 72)
    print("SynthPAI anonymization (TRUNCATED to 25 comments) via run_anonymized()")
    ...
    print(f"  truncate     : YES (original 25-comment cap)")
    ...
    if args.dry_run:
        ...
        print(
            ...
            f"(these will be truncated to 25 by original run_anonymized)."
        )
        print(
            "  Note: input is raw comments; run_anonymized will call "
